...
Target runtime: ~2:55. Narration: ElevenLabs (~410 words, slight slowdown ~0.95x on the forgetting and close beats).
...
- [ ] Codex film-mode revisions landed (advance-clock beat, provenance inspector beat, recall proof verified live)
- [ ] ~15s of "bad tutor" footage for the cold open (no_memory eval condition on screen, or a plain generic chat mock)

---

## [00:00 – 00:14] THE FAILURE COLD OPEN

**Screen:** a plain, generic chat UI (or the no-memory eval condition). Maya's Session 2 message "Where were we?" gets a bland "Hi! What would you like to learn today?"

> Yesterday, Maya told her AI tutor that she panics on chain rule. That she needs worked examples first. That she studies late, after work. Today — it remembers none of it. Every AI assistant you've ever used has amnesia by design.
...
**Screen:** session view. Maya's first message streams. ZOOM to the engram toast the moment it fires, then popout on the new node igniting.

> This brain starts empty. Maya starts talking — and watch the right side. An observer model just caught that: a misconception, typed, scored, and pinned to the moment she said it.
...
**Screen:** Session 2 opens. ZOOM on budget meter filling, then on the tutor's reply citing the misconception.

> Maya returns. Reverie doesn't reload her transcript — it retrieves under a budget. Twelve hundred tokens, scored on relevance, strength, and recency. The tutor already knows what she got wrong last week — she never repeated it.
...
> And the engine contains zero calculus. Swap one script file, and it tutors Spanish — the test suite proves it. Typed memories, event sourcing, dreams, decay, budgeted recall — running on Qwen, on Alibaba Cloud. Your AI shouldn't meet you for the first time, every time.
...
- **Zoom discipline:** zoom only when the VO names a pixel (toast, quote, meter, dimming node). The UI is dark and atmospheric — constant motion reads as chaos. Wide-and-still is the default; zoom is the emphasis.
...
1. Codex lands film-mode revisions → 2. run evals live, fill brackets → 3. rehearse film mode live twice → 4. record footage → 5. generate VO → 6. cut in Kite → 7. export, watch on laptop, submit.
